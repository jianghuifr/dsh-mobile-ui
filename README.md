...
开发时的浏览器验证覆盖了 7 种视口（320/375/393/412/430/768，以及 852×393 横屏），断言：

- 中间列 = 视口宽度，文档不横向滚动，内容不溢出；
- 抽屉开合到位，闭合后完全离屏；
- 输入区字号 ≥ 16px，发送键 44×44，控制行不换行（≤ 60px）；
- 导航按钮 / 发送键 / 输入框在 `elementFromPoint` 下确实可点；
- **设置面板真的能用**：导航 / 首个控件 / 关闭按钮 / 三张外观卡片都可点，标签页切换真的生效，
  内容溢出时能滚动、且不会顶出面板底部。

另有一组**覆盖层生命周期**用例（`cycles`），逐个走完「进去再出来」并断言主界面恢复可交互：

1. 右侧文件管理器：开 → 关；
2. 文件管理器 → 打开一个文件（文件详情）→ 退出；
3. 设置面板：开 → 切标签 → 关；
4. 抽屉与右侧面板嵌套，两种顺序。

还有两组针对「静默」的用例，专治闪烁这一类问题：

- `quiet`：空闲 3 秒内，插件自身属性写入必须为 **0**，且不能有仍在运行的动画——分别在
  对话页和**切到浅色之后**各测一次；
- `vv`：用桩 `visualViewport` **和** 桩 `innerHeight` 把三种平台各跑一遍——地址栏抖动
  （30 次 0↔65px 往复）断言 **0 次写入**；iOS（只缩视觉视口）断言 inset = 300px；
  Android `resizes-content`（两个都缩）断言 inset = **0**，即不重复扣一次键盘；失焦后释放；
- `composer-kb`：用 `setDeviceMetricsOverride` 缩布局视口模拟键盘弹出，断言输入框底边
  **正好贴在可见区底边**（既没被键盘盖住，也没漂到屏幕上方），键盘收起后回到原位；
- `simple`：精简后的移动端——工具排/模型/统计默认收掉、发送键在最右、开关在最左，
  点开关真的展开、再点真的收起；然后**发一条消息让会话不再空白**（顶栏在空会话里是
  `display:none`，不这样做顶栏断言会全部空过），核对顶栏只剩标题 / 右侧边栏 / 标签页，
  访达入口和下载菜单都不在了；最后切到 1280 宽，确认桌面端**一个都没少**；
- `enter`：六条回车契约各用**全新页面**跑一遍（提交会让智能体跑起来，运行中的智能体会改变
  输入框行为并自己增加消息，共用页面会让「消息数不变」的断言失去意义）——
  普通回车换行且不发送、Shift+Enter 换行、Cmd/Ctrl+Enter 发送、发送按钮发送、
  `/re` + 回车由菜单选中命令（并用一个「注册在插件之后」的捕获监听器证明插件没有拦截它）、
  桌面宽度下普通回车仍然发送。
...
- `icons`：真浏览器里逐行核对文件浏览器——每行都画上了图标、App 自带字形被隐藏、
  ...
  `painted=false / glyphHidden=false`，即保留 App 原图标而不是变空。

```
SMOKE PASS — all invariants hold
CYCLES PASS — every overlay returns an interactive main surface
QUIET PASS — the plugin is silent when the app is idle
VV PASS — the inset tracks a real keyboard and ignores browser chrome
COMPOSER-KB PASS — the composer stays above the keyboard, exactly once
ENTER PASS — newline on mobile, submit everywhere it should
SIMPLE PASS — decluttered on mobile, desktop untouched
ICONS PASS
ALL RESOLVE OK
```

后两类断言都是补上来的。它们缺失的那段时间里，点/滑/几何断言全部通过、截图也完全正常，
但实际存在过两次「进去就出不来」的冻结（见上面第 4、5 条坑）。**覆盖层的可交互性必须单独
断言——几何和截图都证明不了它。**
